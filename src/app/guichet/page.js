        ...
        {mode === "login" ? (
          <form key={`login-${formKey}`} onSubmit={handleLogin} className="space-y-4">
            ...
            <Field label="Mot de passe">
              <input autoComplete="new-password" className={inputClass} type="password" value={loginForm.password} onChange={(e) => setLoginForm((f) => ({ ...f, password: e.target.value }))} />
            </Field>
          ...
          <form key={`register-${formKey}`} onSubmit={handleRegister} className="space-y-4">
            ...
            <Field label="Succursale / Agence d'affectation">
              <input autoComplete="new-password" className={inputClass} placeholder="Agence Goma Volcan" value={regForm.agence} onChange={(e) => setRegForm((f) => ({ ...f, agence: e.target.value }))} />
            </Field>
            <Field label="Code du Guichet">
              <input autoComplete="new-password" className={inputClass} placeholder="EQ-GOM1-GUI3" value={regForm.code} onChange={(e) => setRegForm((f) => ({ ...f, code: e.target.value }))} />
            </Field>
            <Field label="Mot de passe de session">
              <input autoComplete="new-password" className={inputClass} type="password" value={regForm.password} onChange={(e) => setRegForm((f) => ({ ...f, password: e.target.value }))} />
            </Field>
            <Alert type="error">{error}</Alert>
            <PrimaryButton type="submit" className="w-full" disabled={loading}>
